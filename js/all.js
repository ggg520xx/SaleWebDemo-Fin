      ...
      cartData.forEach(function (item) {
        str += `<tr>
                    <td>
                      ...
                      <td>
                              ${item.quantity === 1
                                ? `<button type="button" class="removeItem" data-id="${item.id}" disable>-</button>`
                                : `<button type="button" class="removeItem" data-id="${item.id}">-</button>`}
                              <input type="text" id="${item.id}" value="${item.quantity}" disable></input>
                              <button type="button" class="addItem" data-id="${item.id}">
                                +
                              </button>
                      </td>
...
// ---------------------------------------------------------------------------------------------
// 做修改購物車內的東西 和 單一刪除 因為要針對品項做刪除 因此渲染頁面的str要埋id ( 購物車的部分並非產品的
cartList.addEventListener("click", function (e) {
  ...
  if (e.target.classList.value == "addItem") {
  ...
  if (e.target.classList.value == "removeItem") {
...
orderInfoBtn.addEventListener("click", function (e) {
  ...
  // 有購物車資料 資訊也有填寫完整才能通過 下面寫訂購資料的判斷(訂單post的客戶表單資訊)
  ...
  // 寫自己的監聽錯誤code 包裹了 正則函式 但不知道為什麼不起作用
  // --------------------------------------------------------------
  // --------------------------------------------------------------
  ...
  //信箱格式驗證函式使用(在tool.js) -  必要 !  少了這串 會變成格式錯誤 還能發送
